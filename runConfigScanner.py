...
"""
   Load assay/project setting according to the config, we need only enabled workflows
"""
...
"""
def get_vetted_versions(oli_name: str, oli_tags, tracked_versions):
    reported_olives = list(oli_tags)[0] if len(oli_tags) == 1 else list(oli_tags)
    try:
        if 'freeze' in tracked_versions.keys():
            reported_olives = tracked_versions['workflows'][oli_name]
        else:
            reported_olives = list(set(reported_olives, tracked_versions['workflows'][oli_name]))
    finally:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--check', help='Check type', required=False, default="assay")
    ...
    config_check = None
    try:
        check_pattern = settings["checks"][check_type]
        config_check = re.compile(f'({check_pattern})' + '(?P<check>\\S+)')
    except:
        print(f"Failed to compile a search pattern for {check_type} type check")
    ...
    ''' 4. with loaded config file check the projects (or assays) for enabled workflows and construct the report '''
    try:
        config_data = load_config(settings["data"]["assay_config_file"]) if check_type == "assay" else \
                      load_config(settings["data"]["project_config_file"])
    except:
        print("No config file configured in the settings")
        exit(1)

    '''Load and update the version settings, if available'''
    confScanner = configScanner(config_data, olive_info, version_file)
    ...
    if len(vetted_report) > 0:
        ...
        # TODO: check why we do not have arrays for wf versions
        '''HTML output is supported only for assay config, for project-level config only .json is generated'''
        if check_type == "assay":
            html_page = htmlRenderer.convert2page(output_json, java_script)
